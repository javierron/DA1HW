...
species guest skills: [moving, fipa] {
	...
	reflex receive_cfp_from_initiator when: !empty(cfps) {
		...
		switch(proposalFromInitiator.contents at 0){
			match 'dutch'{
				if(interest = proposalFromInitiator.contents[2]){
					if(int(proposalFromInitiator.contents[3]) <= fairPrice ){
						do propose (message: proposalFromInitiator, contents: ['propose', proposalFromInitiator.contents[2], proposalFromInitiator.contents[3]]);	
					}else{
				
					}
				}else{
					if(bool(proposalFromInitiator.contents[1]) = true and int(proposalFromInitiator.contents[3]) < fairPrice){
						do propose (message: proposalFromInitiator, contents: ['propose', proposalFromInitiator.contents[2], proposalFromInitiator.contents[3]]);	
					}else{
	...
	reflex receive_proposal_accept_messages when: !empty(accept_proposals) {
		...
		loop accepted_proposal over: accept_proposals {
			write accepted_proposal.contents;
...
species dutch_auctioneer skills: [moving, fipa] {
	...
	reflex send_cfp_to_participants when: (time mod 50 = 0) {
		...
		if(!sold){
			...
			startPrice <- rnd(200, 500) * 2;
			currentPrice <- startPrice;
			sold <- false;
			sale <- flip(1);
		}
		
		if(sale = 0 and currentPrice < startPrice / 2){
			write 'No proposals above real price were made';
			sold <- true;
			return;
		}else if (sale = 1 and currentPrice < startPrice / 4){
			write 'No proposals above real price were made';
			sold <- true;
			return;
		}
		...
		write '(Time ' + time + '): ' + name + ' sends a cfp message to all participants';
		do start_conversation (to: participants, protocol: 'fipa-contract-net', performative: 'cfp', contents: ['dutch',sale and currentPrice < startPrice / 2, item, currentPrice]);
	}
